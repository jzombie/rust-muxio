...
use muxio_core::rpc::{
    RpcDispatcher, RpcRequest, RpcResponse,
    ...
    C: Send + Sync + Clone + 'static,
{
    ...
    ///
    /// NOTE: the caller holds the dispatcher lock for the whole call. Use the
    /// three primitives (`decode_bytes` / `run_handlers` / `send_responses`)
    /// instead when the handlers can be executed outside the lock, so a handler
    /// that needs the dispatcher (or blocks) cannot deadlock the transport.
    ...
    {
        let requests = self
            .decode_bytes(dispatcher, context.clone(), bytes, on_emit.clone())
            .await?;
        let responses = self.run_handlers(context, requests).await;
        self.send_responses(dispatcher, responses, on_emit)
    }

    /// Stage 1 of transport read processing: install the streaming router,
    /// decode the incoming bytes, collect finalized prebuffered requests, and
    /// wire up stream responders. Requires the dispatcher lock; does not run
    /// any user handlers.
    async fn decode_bytes<'a, E>(
        &self,
        dispatcher: &mut RpcDispatcher<'a>,
        context: C,
        bytes: &[u8],
        on_emit: E,
    ) -> Result<Vec<(u32, RpcRequest)>, RpcServiceEndpointError>
    where
        E: RpcEmit + Send + Sync + Clone + 'static,
    {
        ...
        Ok(finalized_requests)
    }

    /// Stage 2 of transport read processing: execute the finalized requests'
    /// handlers and return their responses.
    ///
    /// MUST be called WITHOUT the dispatcher lock held — a handler that needs
    /// the dispatcher (or awaits on the same runtime) would otherwise deadlock
    /// the transport read loop and stall every other dispatcher user.
    async fn run_handlers(&self, context: C, requests: Vec<(u32, RpcRequest)>) -> Vec<RpcResponse> {
    ...
    /// Stage 3 of transport read processing: encode and emit responses.
    /// Requires the dispatcher lock.
    fn send_responses<'a, E>(
        &self,
        dispatcher: &mut RpcDispatcher<'a>,
