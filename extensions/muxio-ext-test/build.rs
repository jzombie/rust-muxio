...
fn main() {
    ...
    if !tests_dir.is_dir() {
        println!("cargo:rerun-if-changed={}", crate_dir.join("tests").display());
        return;
    }

    let mut discovered = Vec::new();
    collect_test_files(&tests_dir, &tests_dir, &mut discovered).expect("failed to scan tests directory");
    discovered.sort();
