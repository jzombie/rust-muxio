...
async fn test_success_client_server_roundtrip() {
    ...
    let _ = join!(
        endpoint.register_prebuffered(Add::METHOD_ID, |req_bytes, _ctx| async move {
            let req_params = Add::decode_request(&req_bytes)?;
            let sum = req_params.iter().sum();
            let resp_bytes = Add::encode_response(sum)?;
            Ok(resp_bytes)
        }),
        endpoint.register_prebuffered(Mult::METHOD_ID, |req_bytes, _ctx| async move {
        ...
        endpoint.register_prebuffered(Echo::METHOD_ID, |req_bytes, _ctx| async move {
...
async fn test_error_client_server_roundtrip() {
    ...
    endpoint
        .register_prebuffered(Add::METHOD_ID, |_req_bytes, _ctx| async move {
...
async fn test_large_prebuffered_payload_roundtrip_wasm() {
    ...
    endpoint
        .register_prebuffered(Echo::METHOD_ID, |req_bytes: Vec<u8>, _ctx| async move {
            ...
            let req_params = Echo::decode_request(&req_bytes)?;
            Ok(Echo::encode_response(req_params)?)
        })
        .await
        .unwrap();
